...
```tsx
function SimpleView(props: { age: number; name: string }) {
  return <div />;
}

function AlternateView(props: { name: string; age: number }) {
  return <div />;
}
...
```tsx
function PartialView(first: integer, second = 10, third: integer) {
